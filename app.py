...
def create_app(password):
    app = Flask(__name__)
    app.config['SQLALCHEMY_DATABASE_URI'] = f'mysql://root:{password}@localhost/nba_0'
    app.config['SQLALCHEMY_BINDS'] = {'nba_1': f'mysql://root:{password}@localhost/nba_1'}
    ...
    def load_user(user_id):
        engine = create_engine(f'mysql://root:{password}@localhost/nba_0')
    ...
    def signup():
        if request.method == 'POST':
            ...
            engines = [
            create_engine(f'mysql://root:{password}@localhost/nba_0'),
            create_engine(f'mysql://root:{password}@localhost/nba_1')
            ]
    ...
    return app, db  # return both app and db if db operations are needed elsewhere
...
def main():
    ...
    app, db = create_app(args.password)
